...
按提示选择技术栈、是否启用 LLM Judge、输出格式等。初始化会自动生成：

- `.code-spec.json`：项目级配置
- `.trae/rules/project_rules.md`：Trae 规范
- `.cursorrules`：Cursor 规范
- `.github/copilot-instructions.md`：GitHub Copilot 规范
...
```bash
# 查看生效配置
npx tsx /path/to/Code-Spec-Plugin/src/cli.ts config

# 设置全局 LLM API Key（安全，不会进入项目仓库）
npx tsx /path/to/Code-Spec-Plugin/src/cli.ts config set llm.apiKey <your-key> --global

# 设置项目规则包
npx tsx /path/to/Code-Spec-Plugin/src/cli.ts config set rulePacks '["rule-packs/common","rule-packs/typescript","rule-packs/react"]'
```
...
- `rule-packs/common`：通用规范（README 检测等）
- `rule-packs/typescript`：TypeScript 规范（命名、注释、文件长度）
- `rule-packs/react`：React 规范（组件命名、组件文件长度）

新增规则包只需创建 JSON 文件并在配置中引用，无需修改引擎代码。
